...
class Gateway
  ...
  _buildUrl : (apiVersion, servicePath, query) ->
  ...
  bankAccountValidationInteractiveValidate: (accountNumber, sortCode, callback) ->
    ...
    url = @_buildUrl 'v2.00', 'BankAccountValidation/Interactive/Validate', { "AccountNumber": accountNumber, "SortCode": sortCode }
    req = @request.get  url
  ...
  internationalBankAccountValidationInteractiveValidate: (iban, callback) ->
    throw new TypeError unless callback instanceof Function

    url = @_buildUrl 'v1.00', 'InternationalBankValidation/Interactive/Validate', { "IBAN": iban }
    req = @request.get  url
